...
const struct mode modes[] = {
    ...
    { .scaler = SWS_SCALER_BILINEAR },
    { .scaler = SWS_SCALER_NEAREST },
    { .scaler = SWS_SCALER_BICUBIC },
    {0}, // test defaults
};

static FFSFC64 prng_state;
static SwsContext2 *sws[3]; /* reused between tests for efficiency */
...
{
    struct SwsContext *sws_legacy;
    int ret, flags = 0;

    if (mode.flags & SWS_FLAG_BITEXACT)
    ...
    case SWS_SCALER_BILINEAR: flags |= SWS_BILINEAR; break;
    case SWS_SCALER_BICUBIC:  flags |= SWS_BICUBIC;  break;
    case SWS_SCALER_GAUSSIAN: flags |= SWS_SINC;     break;
    case SWS_SCALER_LANCZOS:  flags |= SWS_LANCZOS;  break;
    }

    sws_legacy = sws_getContext(src->width, src->height, src->format,
                    ...
                    const int mse_ref[4])
{
    ...
    if (sws_scale_frame2(sws[0], src, ref) < 0) {
    ...
    sws[1]->scaler = mode.scaler;
    sws[1]->dither = mode.dither;
    if (sws_scale_frame2(sws[1], dst, src) < 0) {
        fprintf(stderr, "Failed %s ---> %s\n", av_get_pix_fmt_name(src->format),
                av_get_pix_fmt_name(dst->format));
        goto error;
    }

    if (sws_scale_frame2(sws[2], out, dst) < 0) {
        fprintf(stderr, "Failed %s ---> %s\n", av_get_pix_fmt_name(dst->format),
...
static int run_file_tests(const AVFrame *ref, FILE *fp, struct options opts)
{
    ...
    while (fgets(buf, sizeof(buf), fp)) {
        ...
        ret = sscanf(buf,
                     " %20s %dx%d -> %20s %dx%d, flags=%u scaler=%u dither=%u, "
                     "MSE={%d %d %d %d}\n",
        ...
        if (src_fmt == AV_PIX_FMT_NONE || dst_fmt == AV_PIX_FMT_NONE ||
            ...
            mode.scaler >= SWS_SCALER_NB || mode.dither >= SWS_DITHER_NB) {
...
bad_option:
    ...
    for (int i = 0; i < 3; i++) {
        sws[i] = sws_alloc_context2();
        if (!sws[i])
            goto error;
        sws[i]->scaler = SWS_SCALER_BILINEAR;
    }
    ...
    if (sws_scale_frame2(sws[0], ref, rgb) < 0)
        goto error;
