...
%elif %1 == 1
        ...
        mov r2d, [execq + SwsOpExec.y]
        lea r4d, [r2d + 1]
IF X || Y,  and r2d, 1
IF Z || W,  and r4d, 1
IF X || Y,  shl r2d, 3
IF Z || W,  shl r4d, 3
IF X || Y,  vbroadcastsd DX, [implq + SwsOpImpl.priv + r2]
IF Z || W,  vbroadcastsd DZ, [implq + SwsOpImpl.priv + r4]
        %define DY DX
        %define DW DZ
%else
        ; matrix is at least 4x4, load all four channels with custom offset
        mov r2d, [execq + SwsOpExec.y]
        mov r6, [implq + SwsOpImpl.priv]
IF Y,   lea r3d, [r2d + 2]
IF Z,   lea r4d, [r2d + 3]
IF W,   lea r5d, [r2d + 5]
IF X,   and r2d, (1 << %1) - 1
...
IF W,   shl r5d, %1+2
    %if %1 == 2
IF X,   VBROADCASTI128 DX, [r6 + r2]
IF Y,   VBROADCASTI128 DY, [r6 + r3]
IF Z,   VBROADCASTI128 DZ, [r6 + r4]
IF W,   VBROADCASTI128 DW, [r6 + r5]
    %else
IF X,   mova DX, [r6 + r2]
IF Y,   mova DY, [r6 + r3]
IF Z,   mova DZ, [r6 + r4]
IF W,   mova DW, [r6 + r5]
        %if (4 << %1) > mmsize
            ...
            %define DZ2 m14
            %define DW2 m15
IF X,       mova DX2, [r6 + r2 + mmsize]
IF Y,       mova DY2, [r6 + r3 + mmsize]
IF Z,       mova DZ2, [r6 + r4 + mmsize]
IF W,       mova DW2, [r6 + r5 + mmsize]
        %endif
    %endif
%endif
        LOAD_CONT r2
...
%macro matrix_row 8 ; res, x, y, z, w, size, offset, row
            vbroadcastss m12, [r2 + %8 * 20 + 0]
            vbroadcastss m13, [r2 + %8 * 20 + 4]
            vbroadcastss m14, [r2 + %8 * 20 + 8]
IF %6 > 3,  vbroadcastss m15, [r2 + %8 * 20 + 12]
IF %7,      vbroadcastss %1,  [r2 + %8 * 20 + 16]
            mulps m12, %2
            mulps m13, %3
            mulps m14, %4
IF %6 > 3,  mulps m15, %5
...
%macro matrix_inner 6 ; x, y, z, w, size, offset
            matrix_row m8,  %1, %2, %3, %4, %5, %6, 0
            matrix_row m9,  %1, %2, %3, %4, %5, %6, 1
            matrix_row m10, %1, %2, %3, %4, %5, %6, 2
IF %5 > 3,  matrix_row m11, %1, %2, %3, %4, %5, %6, 3
            mova %1, m8
            mova %2, m9
            mova %3, m10
IF %5 > 3,  mova %4, m11
...
%macro matrix_off 2 ; size, offset
op matrix%1_off%2
        mov r2, [implq + SwsOpImpl.priv] ; address of matrix
        LOAD_CONT r3
        matrix_inner mx,  my,  mz,  mw,  %1, %2
        matrix_inner mx2, my2, mz2, mw2, %1, %2
        CONTINUE r3
%endmacro
...
%define NOP(J) (J == %6 && !COL(J)) ; true if J is untouched input component
    ...
    ; load weights
    IF COL(0),  vbroadcastss m12,  [r2 + %6 * 20 + 0]
    IF COL(1),  vbroadcastss m13,  [r2 + %6 * 20 + 4]
    IF COL(2),  vbroadcastss m14,  [r2 + %6 * 20 + 8]
    IF COL(3),  vbroadcastss m15,  [r2 + %6 * 20 + 12]

    ; initialize result vector as appropriate
    %if COL(4) ; offset
        vbroadcastss %1,   [r2 + %6 * 20 + 16]
    %elif NOP(0)
...
%macro linear_dot3 0
op dot3
        mov r2, [implq + SwsOpImpl.priv]
        LOAD_CONT r3
        vbroadcastss m12,  [r2 + 0]
        vbroadcastss m13,  [r2 + 4]
        vbroadcastss m14,  [r2 + 8]
        mulps mx, m12
        mulps m8, my, m13
        mulps m9, mz, m14
        addps mx, m8
