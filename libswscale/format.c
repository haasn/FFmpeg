...
static AVRational *generate_bayer_matrix(const int size_log2)
{
    ...
    AVRational *m = av_malloc(sizeof(*m) * num_entries);
                      ...
                      const SwsPixelType type, const SwsFormat fmt)
{
    ...
    if (mode == SWS_DITHER_AUTO && bpc >= 16)
        mode = SWS_DITHER_NONE;

    /* Hardcode maximum size matrix for now; in theory we could adjust this
     * based on the expected level of precision in the output */
    dither.size_log2 = mode == SWS_DITHER_NONE ? 0 : av_log2(SWS_MAX_DITHER);
