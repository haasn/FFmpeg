...
extern const SwsOpTable ff_sws_op_tmpl_int_table_avx2_32bpc;
extern const SwsOpTable ff_sws_op_tmpl_float_table_avx2_32bpc;
#endif
...
static const SwsOpTable * const sws_op_tables[] = {
    ...
    &ff_sws_op_tmpl_int_table_avx2_32bpc,
    &ff_sws_op_tmpl_float_table_avx2_32bpc,
#endif
...
static SwsPixelType pixel_type_to_uint(SwsPixelType type)
{
    switch (pixel_type_size(type)) {
    case 1: return SWS_PIXEL_U8;
    case 2: return SWS_PIXEL_U16;
    case 4: return SWS_PIXEL_U32;
    }

    return SWS_PIXEL_INVALID;
}
...
static inline bool op_match(const SwsOp *op, const SwsOp *ref, int *score)
{
    SwsPixelType type = op->type;

    switch (op->op) {
    /* These operations can be performed on integers instead of floats with
     * identical results, so convert the type before checking */
    case SWS_OP_READ:
    case SWS_OP_WRITE:
    case SWS_OP_SWAP_BYTES:
    case SWS_OP_SWIZZLE:
        type = pixel_type_to_uint(op->type);
        break;
    }
    ...
    /* Base score per matched operation */
    *score = 10;
    ...
    case SWS_OP_READ:
    case SWS_OP_WRITE:
        return op->rw.elems  == ref->rw.elems &&
    ...
    case SWS_OP_CLEAR:
        ...
        for (int i = 0; i < 4; i++) {
            if (op->clear.value[i].den && !ref->clear.value[i].den)
                return false;
        }
        return true;
    case SWS_OP_LSHIFT:
    ...
    case SWS_OP_SWIZZLE:
        for (int i = 0; i < 4; i++) {
            if (op->comps.flags[i] & SWS_COMP_GARBAGE)
                continue;
            if (op->swizzle.in[i] != ref->swizzle.in[i])
                return false;
        }
        return true;
    case SWS_OP_CONVERT:
        return op->convert.to     == ref->convert.to &&
               op->convert.expand == ref->convert.expand;
    case SWS_OP_DITHER:
        return op->dither.size_log2 == ref->dither.size_log2;
    case SWS_OP_CLAMP:
        return true;
    case SWS_OP_LINEAR:
...
/* Returns the score of the matched op entry, or 0 for no match */
static int match_op_entry(const SwsOpList *ops, const SwsOpEntry *entry)
{
    int score, score_sum = 0;
    if (entry->num_ops > ops->num_ops)
        return false;

    for (int i = 0; i < entry->num_ops; i++) {
        if (!op_match(&ops->ops[i], &entry->ops[i], &score))
            return 0;
        score_sum += score;
    }
...
/* Returns the score of the matched operation, or 0 for no match */
static int lookup_op_entry(const SwsOpList *ops, const int flags,
                           const SwsOpEntry **out_entry)
{
    const SwsOpEntry *best = NULL;
    const int cpu_flags = av_get_cpu_flags();
    int score, best_score = 0;

    /* Greedy algorithm, tries to match the longest entry */
    for (int i = 0; i < FF_ARRAY_ELEMS(sws_op_tables); i++) {
        const SwsOpTable *table = sws_op_tables[i];
        if (table->cpu_flags & ~cpu_flags)
            continue;
        for (int j = 0; j < table->num_entries; j++) {
            const SwsOpEntry *entry = &table->entries[j];
            score = match_op_entry(ops, entry);
            if (!score)
                continue;
            score += table->score;
            if (score > best_score) {
...
static int op_list_update_comps(SwsOpList *ops)
{
    ...
    /* Forwards pass, propagetes knowledge about the incoming pixel values */
    ...
    for (int n = ops->num_ops - 1; n >= 0; n--) {
        ...
        /* Strip metadata from unused components for sanity */
        for (int i = 0; i < 4; i++) {
            if (next.unused[i])
                op->comps.flags[i] = SWS_COMP_GARBAGE;
        }
    ...
    do {
        ...
        for (int n = 0; n < ops->num_ops;) {
            ...
            case SWS_OP_SWIZZLE:
                ...
                /* Don't swizzle unused elements */
                for (int i = 0; i < 4; i++) {
                    if (next->comps.unused[i]) {
                        changed |= op->swizzle.in[i] != i;
                        op->swizzle.in[i] = i;
                    }
                }
                ...
                /* Prefer swizzling on smaller element size, or when it allows
                 * us to merge the read and convert-to-float steps */
                if (next->op == SWS_OP_CONVERT &&
                    pixel_type_size(next->convert.to) < pixel_type_size(op->type))
                {
                    op->type = next->convert.to;
                    swap_ops(op, next);
                    progress = true;
                    continue;
                } else if (prev->op == SWS_OP_CONVERT &&
...
typedef struct SwsOpPass {
    ...
    sws_read_t read;
    sws_read_t read_n;
    void *read_priv; /* also used for read_write */
...
#define SWS_MAX_POST_OPS 16
    sws_generic_op_t ops[SWS_MAX_POST_OPS];
    void *ops_priv[SWS_MAX_POST_OPS];
    int num_ops;
} SwsOpPass;
...
static void op_pass_free(void *ptr)
{
    ...
    av_free(op->read_priv);
    av_free(op->write_priv);
    av_free(op->filter_priv);
    for (int i = 0; i < op->num_ops; i++)
        av_free(op->ops_priv[i]);
    av_free(op);
}

static av_const bool img_aligned(SwsImg img)
{
    for (int i = 0; i < 4; i++) {
        uintptr_t ptr = (uintptr_t) img.data[i];
        if (ptr & (SWS_ALIGNMENT - 1))
            return false;
    }
...
static av_always_inline void run(const SwsImg *out_base, const SwsImg *in_base,
                                 ...
                                 const SwsOpPass op)
{
    ...
    const int base_w = w & ~(SWS_CHUNK_SIZE - 1);
    const int rest_w = w - base_w;

    const sws_read_t  read  = img_aligned(in)  ? op.read  : op.read_n;
    const sws_write_t write = img_aligned(out) ? op.write : op.write_n;

    /**
     * Keep this pinned at a static location in memory. This is not just
     * beneficial for caching, but more importantly, it ensures that the
     * memory never contains uninitialized stack data, which would be bad
     * if somehow accidentally read from.
     */
    static _Thread_local tmp_t tmp;

    for (int line = 0; line < h; line++) {
        const int yy = y + line;
        const plane_t *in0 = (const plane_t *) in.data[0];
        const plane_t *in1 = (const plane_t *) in.data[1];
        const plane_t *in2 = (const plane_t *) in.data[2];
        const plane_t *in3 = (const plane_t *) in.data[3];
        plane_t *out0 = (plane_t *) out.data[0];
        plane_t *out1 = (plane_t *) out.data[1];
        plane_t *out2 = (plane_t *) out.data[2];
        plane_t *out3 = (plane_t *) out.data[3];

        for (int x = 0; x < base_w; x += SWS_CHUNK_SIZE) {
            read(&tmp, SWS_CHUNK_SIZE, in0, in1, in2, in3, op.read_priv);
            for (int i = 0; i < op.num_ops; i++)
                op.ops[i](&tmp, yy, op.ops_priv[i]);
            write(&tmp, SWS_CHUNK_SIZE, out0, out1, out2, out3, op.write_priv);
        ...
        if (rest_w) {
            tmp_t tmp;
            op.read_n(&tmp, rest_w, in0, in1, in2, in3, op.read_priv);
            for (int i = 0; i < op.num_ops; i++)
                op.ops[i](&tmp, yy, op.ops_priv[i]);
            op.write_n(&tmp, rest_w, out0, out1, out2, out3, op.write_priv);
        }
...
static void run_generic(const SwsImg *out, const SwsImg *in, int y,
                        int h, const SwsPass *pass)
{
    run(out, in, y, h, pass, *(const SwsOpPass *) pass->priv);
}

/* Wrapper that fixes certain parameters for constant folding purposes */
#define WRAP_RUN(FIELD, VAL)                                                    \
static void run_##FIELD##_##VAL(const SwsImg *out, const SwsImg *in,            \
                                int y, int h, const SwsPass *pass)              \
{                                                                               \
    SwsOpPass op = *(const SwsOpPass *) pass->priv;                             \
    op.FIELD = (VAL);                                                           \
    run(out, in, y, h, pass, op);                                               \
}
...
/* Special case runner for a single read-write op */
static void run_read_write(const SwsImg *out_base, const SwsImg *in_base,
                           const int y, const int h, const SwsPass *pass)
{
    const SwsOpPass op = *(const SwsOpPass *) pass->priv;
    SwsImg in  = ff_sws_img_shift(*in_base,  y);
    SwsImg out = ff_sws_img_shift(*out_base, y);
    const int w = pass->width;

    for (int line = 0; line < h; line++) {
        op.read_write(w, out.data[0], in.data[0], out.data[1], in.data[1],
                         out.data[2], in.data[2], out.data[3], in.data[3],
                         op.read_priv);
        for (int i = 0; i < 4; i++) {
            in.data[i]  += in.linesize[i];
            out.data[i] += out.linesize[i];
...
static int rw_op_stride(const SwsOp *op)
{
    /* ensure that monow/monob can fit */
    static_assert(SWS_CHUNK_SIZE >= 8, "SWS_CHUNK_SIZE must be at least 8");
    const int pixels = SWS_CHUNK_SIZE >> op->rw.frac;
    const int elems  = op->rw.planar ? 1 : op->rw.elems;
    const int size   = pixel_type_size(op->type);
    return elems * size * pixels;
}
                        ...
                        SwsPass *input, SwsPass **output)
{
    ...
    SwsOpList rest;
    SwsOpPass *c;
    sws_filter_run_t run_fn = run_generic;
    SwsPass *pass;
    int ret, op_idx;
    ...
    if (!c)
        return AVERROR(ENOMEM);
    c->in_stride  = rw_op_stride(input_op);
    c->out_stride = rw_op_stride(output_op);

    /* Compile the sequence of ops into a sequence of op entries */
    op_idx = 0;
    rest = *ops;
    while (rest.num_ops) {
        ...
        int score = lookup_op_entry(&rest, flags, &entry);
        int num_matched = score ? entry->num_ops : 0;
        const char *match_type;

        if (!score) {
            av_log(ctx, AV_LOG_WARNING, "No compatible implementation found "
                   "for operations:\n");
        ...
        is_first = op_idx == 0;
        is_last  = num_matched == rest.num_ops;

        if (is_first && is_last) {
            av_log(ctx, AV_LOG_DEBUG, "Using special converter");
            c->read_write = entry->read_write;
            c->read_priv  = priv;
            run_fn = run_read_write;
            goto setup_pass;
        } else if (is_first) {
            c->read      = entry->read;
            c->read_n    = entry->read_n;
            c->read_priv = priv;
            match_type   = "Input";
        } else if (is_last) {
            c->write      = entry->write;
            c->write_n    = entry->write_n;
            c->write_priv = priv;
            match_type    = "Output";
        } else {
            const int idx = c->num_ops++;
            av_assert1(idx < FF_ARRAY_ELEMS(c->ops));
            c->ops[idx]      = entry->op;
            c->ops_priv[idx] = priv;
            match_type       = "Generic";
        }

        av_log(ctx, AV_LOG_DEBUG, "%s function matched %d ops with score %d\n",
               match_type, num_matched, score);

        op_idx += num_matched;
        rest.ops += num_matched;
        rest.num_ops -= num_matched;
        av_assert2(rest.num_ops >= 0);
    }

    switch (c->num_ops) {
    case 0:  run_fn = run_num_ops_0; break;
    case 1:  run_fn = run_num_ops_1; break;
    case 2:  run_fn = run_num_ops_2; break;
    case 3:  run_fn = run_num_ops_3; break;
    case 4:  run_fn = run_num_ops_4; break;
    }
...
setup_pass:
    ...
    if (run_fn != run_generic)
        av_log(ctx, AV_LOG_DEBUG, "Using dedicated dispatch function\n");
