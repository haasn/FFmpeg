...
static av_cold int preinit(AVFilterContext *ctx)
{
    ...
    ret = av_opt_set_int(scale->avs, "threads", 0, 0);
    if (ret < 0)
        return ret;
...
static av_cold int init(AVFilterContext *ctx)
{
    ...
    */
    ...
    ret = av_opt_get_int(scale->avs, "threads", 0, &threads);
    if (ret < 0)
        return ret;
    if (!threads)
        av_opt_set_int(scale->avs, "threads", ff_filter_get_nb_threads(ctx), 0);
...
static av_cold void uninit(AVFilterContext *ctx)
{
    ...
    avscale_free_context(&scale->avs);
}
...
static int query_formats(AVFilterContext *ctx)
{
    ...
    while ((desc = av_pix_fmt_desc_next(desc))) {
        ...
        if (avscale_test_format(pix_fmt, 0)) {
...
static int config_props(AVFilterLink *outlink)
{
    ...
    av_opt_get(scale->avs, "sws_flags", 0, &flags_val);
...
{
    const AVClass *c = *iter ? NULL : avscale_get_class();
