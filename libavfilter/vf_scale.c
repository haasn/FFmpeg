...
static av_cold int preinit(AVFilterContext *ctx)
{
    ...
    scale->sws = sws_alloc_context2();
    if (!scale->sws)
        return AVERROR(ENOMEM);
    scale->sws->flags |= SWS_FLAG_ALIAS; /* preserve backwards compatibility */

    // set threads=0, so we can later check whether the user modified it
    ret = av_opt_set_int(scale->sws, "threads", 0, 0);
    if (ret < 0)
        return ret;
...
static av_cold int init(AVFilterContext *ctx)
{
    ...
    */
    ...
    ret = av_opt_get_int(scale->sws, "threads", 0, &threads);
    if (ret < 0)
        return ret;
    if (!threads)
        av_opt_set_int(scale->sws, "threads", ff_filter_get_nb_threads(ctx), 0);
...
static int query_formats(AVFilterContext *ctx)
{
    ...
    if (scale->out_range != AVCOL_RANGE_UNSPECIFIED) {
        formats = ff_make_formats_list_singleton(scale->out_range);
    } else {
        formats = ff_all_color_spaces();
        for (int i = 0; i < formats->nb_formats; i++) {
...
scale:
    ...
    ret = sws_scale_frame2(scale->sws, out, in);
    av_frame_free(&in);
...
{
    const AVClass *c = *iter ? NULL : sws_get_class2();
