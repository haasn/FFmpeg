...
static int movie_query_formats(AVFilterContext *ctx)
{
    ...
    for (i = 0; i < ctx->nb_outputs; i++) {
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            if ((ret = ff_formats_ref(ff_make_format_list(list), &outlink->incfg.color_spaces)) < 0)
                return ret;
            list[0] = c->color_range;
            if ((ret = ff_formats_ref(ff_make_format_list(list), &outlink->incfg.color_ranges)) < 0)
                return ret;
            break;
        case AVMEDIA_TYPE_AUDIO:
            list[0] = c->format;
